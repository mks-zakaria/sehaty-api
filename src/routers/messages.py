...
def list_threads(patient: User = Depends(_require_patient)) -> list[ThreadView]:
    """The calling patient's inbox, newest-activity first, with per-thread unread."""
    return MessagingController.list_threads_for_patient(patient.id)
...
def start_thread(
    ...
    return MessagingController.start_or_get_thread(patient.id, body.doctor_id)
...
def get_thread(
    ...
    return MessagingController.get_thread(patient.id, thread_id)
...
def post_message(
    ...
    return MessagingController.post_message(patient.id, thread_id, body.body)
