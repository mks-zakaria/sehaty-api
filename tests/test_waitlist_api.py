...
def _subscribe(db: sessionmaker[Session], doctor_id: int) -> None:
    """Give the doctor a live subscription, as accreditation does in production.

    The waitlist is part of the booking engine, so joining one checks the
    doctor is entitled to it. Registration alone does not create a
    subscription — an admin accrediting the doctor opens the trial.
    """
    with db() as session:
        plan = session.execute(select(Plan).limit(1)).scalar_one_or_none()
        if plan is None:
            plan = Plan(code="wl", name="Waitlist test", price_month=199, is_active=True)
            session.add(plan)
            session.commit()
        session.add(
            Subscription(
...
def _doctor(client: TestClient, db: sessionmaker[Session], suffix: str) -> tuple[int, str]:
    reg = client.post(
...
def test_a_released_slot_reaches_the_patient_who_can_then_take_it(
    ...
    doctor_id, doctor_token = _doctor(client, db, "1")
...
def test_me_never_shows_another_patients_entries(
    ...
    doctor_id, _ = _doctor(client, db, "2")
...
def test_me_requires_a_patient(client: TestClient, db: sessionmaker[Session]) -> None:
    _, doctor_token = _doctor(client, db, "3")
...
def test_declining_keeps_the_place_in_the_queue(
    ...
    doctor_id, doctor_token = _doctor(client, db, "4")
...
def test_cannot_queue_for_a_doctor_without_a_live_subscription(
    client: TestClient, db: sessionmaker[Session]
) -> None:
    """The waitlist is part of the booking engine, which is what the pack buys.

    Registration alone does not open a subscription — accreditation does — so a
    doctor who has not been accredited (or has lapsed) cannot be queued for.
    Their public page stays live either way; only the engine is gated.
    """
    reg = client.post(
        "/api/v1/auth/doctor/register",
        json={
            "email": "wl-unpaid@clinic.ma",
            "password": "wl-pw-12345",
            "full_name": "Dr Unpaid",
            "slug": "dr-unpaid",
            "license_no": "LIC-WL-UNPAID",
            "phone": "+212618009999",
        },
    )
    assert reg.status_code == 201, reg.text
    _, patient_token = _patient(db, "wl-hopeful@sehaty.ma")

    join = client.post(
        "/api/v1/waitlist",
        headers=_auth(patient_token),
        json={"doctor_id": int(reg.json()["id"])},
    )
